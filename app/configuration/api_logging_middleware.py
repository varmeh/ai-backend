...
class APILoggingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        # Insert your pre-processing logic here
        request_id = str(uuid4())
        logging_dict = {
            _request_Id_key: request_id  # X-API-REQUEST-ID maps each request-response to a unique ID
        }
        logging_dict["request"] = await self._log_request(request, request_id)

        start_time = time.perf_counter()

        response = await call_next(request)

        # Insert your post-processing logic here
    ...
    async def _log_request(self, request: Request, request_id: str) -> dict:
        ...
        if (
            ...
            if "auth" in request.url.path:
                ...
                await self._receive_body_in_middleware(request)
                try:
    ...
    async def _log_response(
        ...
        if not LOG_API_DETAILED:
            ...
            logging_dict["response"]["body"] = await self._extract_and_set_body(
                response
            )
            logger.info(
    ...
    async def _receive_body_in_middleware(self, request: Request):
        """
    ...
    async def _extract_and_set_body(self, response: Response) -> Union[str, dict]:
